...
When no curated benchmark file is uploaded, the benchmark source selector now prefers accepted reviewed candidates over provisional external-evidence candidates. Provisional candidates remain available only as an explicit fallback, and snapshot/report output records the `source_mode`, `is_provisional`, and `is_reviewed_candidate` flags so later analyses can distinguish reviewed references from triage-only references. The row-level source audit, summary, case summary, case decision import/validation/outcomes/outcome summary/closure queue/readiness impact/readiness impact summary/dataset impact/dataset impact cases/dataset impact case checklist/dataset impact report/closure checklist, case checklist, action queue and checklist also record `source_claim_status` and `can_support_independent_claim`, separating uploaded curated references from review-qualified candidate references and blocked provisional references. The readiness gate consumes this audit with decision outcomes: provisional references remain `P0` blockers and review-qualified accepted candidates remain `P2` independence-review items until source-audit case decisions close them; `cleared`, `replaced` and `source-ready` cases stop producing source-audit readiness issues, while `blocked`, `pending`, `held` and unknown outcomes continue as P0/P1/P2 issues. A readiness impact export compares the original source issue with the decision-adjusted source issue per case, so reviewers can see which cases were cleared by decision and which still block claims. Its summary aggregates cleared/open cases, original/adjusted P0/P1/P2 counts and net blocker delta. The dataset impact export then aggregates those source decision effects per Top-N dataset interpretation, including source blockers, review cases and source-gate mismatches. The dataset impact cases export joins each Top-N case interpretation with source decision impact, so reviewers can identify the exact enzyme or structure cases whose claim status diverges from source readiness. The dataset impact case checklist keeps only blocker, review and mismatch case rows as Markdown actions, while the dataset impact report summarizes the source gate, Top-N source-impact table and case actions for archive-ready review. The case summary, case decision import/validation/outcomes/outcome summary/closure queue/closure checklist and case checklist group those source-only blockers and review needs by `benchmark_id`, then summarize blocked, pending, held, replaced, cleared and source-ready cases into a closure status, machine-readable action queue and reviewer checklist so batch benchmark runs can prioritize affected enzyme or structure cases first.
