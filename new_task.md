...
- [x] Benchmark reference source control：无 curated 文件时优先 accepted reviewed candidate，未审核 external-evidence candidate 只作为显式 provisional fallback，并在 snapshot/report 记录 source mode。
...
- [x] 单元测试和全量测试覆盖，最近一次验证：`227 passed`。
...
- 已落地 benchmark reference source control：在无 curated benchmark 文件时，accepted reviewed candidate 优先于 provisional external-evidence candidate；provisional 路径必须显式开启，并会在 snapshot/report 标记为 provisional。
