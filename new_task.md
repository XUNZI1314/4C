...
- [x] Benchmark reference source control：无 curated 文件时优先 accepted reviewed candidate，未审核 external-evidence candidate 只作为显式 provisional fallback，并在 snapshot/report/source audit/summary/case summary/action queue/checklist 与 readiness gate 记录 source mode 和 claim 安全状态。
...
- [x] 单元测试和全量测试覆盖，最近一次验证：`233 passed`。
