...
- Catalytic pocket benchmark：上传 curated catalytic residues 后输出 Top-1 / Top-3 / Top-5 coverage、case/dataset summary、best hit rank、missed residues，并可按整体、case、dataset 和 residue 层面对比 current 与 no-p2rank / no-literature / no-evidence-route / no-conservation-rerank 的 coverage loss，同时生成 remediation queue。
