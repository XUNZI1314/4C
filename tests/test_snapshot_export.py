...
def test_snapshot_extra_preserves_nested_detection_payloads():
    ...
    assert any("基准参考来源审计行动队列: 3 行 / 阻断项 3 / 复核 0" in line for line in summary_lines)
    assert any("基准参考来源审计案例决策结果: 3 行 / 阻断 1 / 待处理 1 / 已清除 1" in line for line in summary_lines)
    assert any("基准参考候选复核: 4 行 / P1 2 / P2 2 / 清单 可用" in line for line in summary_lines)
    assert any("基准参考候选复核决策: 4 行 / 校验阻断 1 / 已接受动作 3 / 已接受参考 1" in line for line in summary_lines)
    assert any("催化口袋基准: 参考 3 / Top-1 0.667 (Top-1 部分命中) / Top-3 1.000 (Top-N 完全命中)" in line for line in summary_lines)
    assert any("基准参考整理质量: 2 问题 / 汇总 1 行 / 清单 可用" in line for line in summary_lines)
    assert any("基准参考就绪: 阻断 / 阻断项 6 / 复核 2 / 来源审计 3 / 队列 8 / 清单 可用" in line for line in summary_lines)
    assert any("基准解释: 3 行 / Top-1 结论 阻断 / Top-3 结论 阻断" in line for line in summary_lines)
    assert any("基准案例解释矩阵汇总: 阻断 / 可用 1" in line for line in summary_lines)
    assert any("基准来源审计决策数据集影响行动汇总: 3 行 / 动作 4 / P0 组 2 / 不匹配项 1 / Top P0 来源门控不匹配" in line for line in summary_lines)
    assert any("基准来源审计决策数据集影响产物: 6 个文件 / 字节 12345 / 哈希 6" in line for line in summary_lines)
    assert any("基准数据集解释队列: 2 行 / 阻断项 1 / 复核 1 / 清单 可用 / 报告 可用" in line for line in summary_lines)
    assert any("催化基准数据集: 案例 2 / 汇总行 2" in line for line in summary_lines)
    assert any("催化基准变体案例: 4 行 / 数据集行 2" in line for line in summary_lines)
    assert any("催化基准修复队列: 2 行 / 汇总 1 行 / 清单 可用" in line for line in summary_lines)
    joined_summary = "\n".join(summary_lines)
    for forbidden_label in [
        "Benchmark reference",
        "Benchmark source-audit",
        "Benchmark interpretation",
        "Benchmark case interpretation",
