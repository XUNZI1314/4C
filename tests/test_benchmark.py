...
def test_benchmark_reference_source_audit_checklist_marks_risky_rows():
    reference_df = pd.DataFrame(
        [
            {
                "benchmark_id": "enzyme-a",
    ...
    checklist = build_pocket_benchmark_reference_source_audit_checklist_markdown(summary, source_audit)

    assert "Benchmark reference source audit checklist" in checklist
    assert "Source Status Summary" in checklist
    assert "`blocked-provisional`" in checklist
...
def test_benchmark_reference_source_audit_case_checklist_groups_actions_by_case():
    reference_df = pd.DataFrame(
        [
            {
                "benchmark_id": "enzyme-a",
    ...
    checklist = build_pocket_benchmark_reference_source_audit_case_checklist_markdown(case_summary, queue)

    assert "Benchmark reference source audit case checklist" in checklist
    assert "Case Actions" in checklist
    assert "case `enzyme-a` `P0` `provisional_reference_source`" in checklist
...
def test_benchmark_reference_source_audit_case_decision_outcomes_apply_validated_decisions():
    case_summary = pd.DataFrame(
        [
            {
                "benchmark_id": "enzyme-a",
...
enzyme-a,accept,Alice,uploaded-curated,yes,,independent source verified
"""
    decisions, _metadata = parse_pocket_benchmark_reference_source_audit_case_decision_table(decision_text)
    validation = build_pocket_benchmark_reference_source_audit_case_decision_validation(decisions, case_summary)

    outcomes = build_pocket_benchmark_reference_source_audit_case_decision_outcomes(case_summary, decisions, validation)

    statuses = dict(zip(outcomes["benchmark_id"], outcomes["applied_status"]))
...
def test_benchmark_reference_source_audit_case_decision_dataset_impact_counts_topn_source_blockers():
    case_interpretation = pd.DataFrame(
        [
            {
                "benchmark_id": "enzyme-a",
    ...
    dataset_impact = build_pocket_benchmark_reference_source_audit_case_decision_dataset_impact(
        case_interpretation,
        impact,
    )

    row = dataset_impact.iloc[0]
    assert int(row["top_n"]) == 1
    assert int(row["case_count"]) == 4
    assert int(row["claim_ready_case_count"]) == 2
    assert int(row["source_tracked_case_count"]) == 4
    assert int(row["source_open_case_count"]) == 2
    assert int(row["source_blocker_case_count"]) == 1
    assert int(row["source_review_case_count"]) == 1
    assert int(row["source_cleared_case_count"]) == 1
    assert int(row["source_ready_case_count"]) == 1
    assert int(row["dataset_blocked_by_source_cases"]) == 1
    assert int(row["dataset_review_by_source_cases"]) == 1
    assert int(row["source_gate_mismatch_case_count"]) == 0
...
def test_benchmark_reference_source_audit_case_decision_dataset_impact_case_checklist_lists_actions():
    detail = pd.DataFrame(
        [
            {
                ...
                "recommended_action": "Resolve source blocker.",
            },
            {
            ...
            },
            {
                ...
                "adjusted_source_priority": "",
                "adjusted_source_issue_type": "",
                "source_impact_status": "source-cleared",
                "source_action_status": "closed",
                "source_gate_mismatch": False,
                "recommended_action": "Keep decision exports.",
            },
        ]
    )

    checklist = build_pocket_benchmark_reference_source_audit_case_decision_dataset_impact_case_checklist_markdown(
        detail
    )

    assert checklist.startswith("# Benchmark source-audit decision dataset impact case checklist")
    assert "| blocker | source-blocked | 1 | 1 | 0 |" in checklist
    assert "| blocker | source-gate-mismatch | 1 | 1 | 1 |" in checklist
...
def test_benchmark_reference_source_audit_case_decision_dataset_impact_action_queue_filters_actions():
    detail = pd.DataFrame(
        [
            {
                "impact_case_id": "BRSDIC-001",
            ...
            },
            {
                ...
                "best_rank": 3,
                "adjusted_source_priority": "P2",
                "adjusted_source_issue_type": "source-review",
                "readiness_impact": "decision-open",
    ...
    queue = build_pocket_benchmark_reference_source_audit_case_decision_dataset_impact_action_queue(detail)

    assert queue["action_id"].tolist() == ["BRSDIA-001", "BRSDIA-002", "BRSDIA-003"]
    assert queue["priority"].tolist() == ["P0", "P0", "P2"]
    assert queue["benchmark_id"].tolist() == ["enzyme-a", "enzyme-b", "enzyme-c"]
    assert queue["source_impact_status"].tolist() == [
        "source-blocked",
